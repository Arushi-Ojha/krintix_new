...
const PHASES: Record<EvolutionPhase, {
  ...
  LEGACY: {
    ...
    executiveTitle: "Fragmented & Static",
    description: "Traditional web development relies on disjointed tools and manual updates. This approach ignores scalable architecture and custom AI software development.",
    businessOutcome: "High maintenance costs and restricted scalability. You are constantly running to stand still.",
  ...
  STANDARD: {
    ...
    executiveTitle: "Linear Constraints",
    description: "While visually appealing, platform builders lack deep AI integration and custom backend capabilities. They solve immediate needs but create long-term technical debt.",
    businessOutcome: "Performance plateaus. As your user base grows, the lack of cloud-native architecture forces a complete rebuild.",
  ...
  KRINTIX_ARE: {
    ...
    executiveTitle: "AI-Powered SaaS",
    description: "AI development is the process of building intelligent systems that automate workflows and improve decision making. We engineer bespoke platforms designed for infinite scale.",
    businessOutcome: "Seamless business process automation and maximum conversion efficiency. Your technology becomes your competitive moat.",
    accentColor: "#0d1754", // Navy Blue
    visualNodes: [
         ...
         <svg viewBox="0 0 400 400" className="w-full h-full overflow-visible pointer-events-none">
            ...
            {phase === "KRINTIX_ARE" && (
              <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                {/* Orbital Rings - Navy, White, Black to match palette */}
                {[
                  { scale: 1, color: "stroke-[#0d1754]/50" }, 
                  { scale: 1.3, color: "stroke-white/40" }, 
                  { scale: 1.6, color: "stroke-black/80" }
                ].map((ring, i) => (
                  <motion.circle
                    key={i}
                    cx="200" cy="200" r={60 * ring.scale}
                    className={`fill-none stroke-[1.5] ${ring.color}`}
                    animate={{ rotate: i % 2 === 0 ? 360 : -360 }}
                    transition={{ duration: 10 + i * 5, repeat: Infinity, ease: "linear" }}
                  />
                ))}
                
                {/* Hyper-speed Particles - Navy, Yellow, White */}
                {current.visualNodes.map((node, i) => {
                  const lineColors = ["stroke-[#0d1754]/50", "stroke-yellow-400/50", "stroke-white/50"];
                  const particleColors = ["#0d1754", "#eab308", "#ffffff"];
                  return (
                    <React.Fragment key={i}>
                      <motion.line 
                        x1="200" y1="200" x2={node.x} y2={node.y}
                        className={`${lineColors[i % 3]} stroke-[1.5]`}
                        strokeDasharray="4 4"
                        animate={{ strokeDashoffset: [0, -20] }}
                        transition={{ duration: 0.5, repeat: Infinity, ease: "linear" }}
                      />
                      <ParticleFlow x1={200} y1={200} x2={node.x} y2={node.y} color={particleColors[i % 3]} speed={0.8} delay={i * 0.2} />
                      <ParticleFlow x1={node.x} y1={node.y} x2={200} y2={200} color={particleColors[(i + 1) % 3]} speed={1.2} delay={i * 0.3} />
                    </React.Fragment>
                  );
                })}

                {/* Core incorporates Navy, Black, Yellow, and White */}
                <circle cx="200" cy="200" r="40" className="fill-[#0d1754]/20 stroke-[#0d1754] stroke-[2]" filter="url(#engine-glow)" />
                <circle cx="200" cy="200" r="35" className="fill-black/80 stroke-black stroke-[2]" />
                <motion.circle 
                  cx="200" cy="200" r="30" 
                  className="fill-none stroke-yellow-400/80 stroke-[2] stroke-dasharray-[5,10]"
                  animate={{ rotate: 360, scale: [1, 1.1, 1] }}
                  transition={{ duration: 4, repeat: Infinity, ease: "linear" }}
                />
                <circle cx="200" cy="200" r="15" className="fill-white/20 stroke-white stroke-[1]" />
              </motion.g>
            )}
        ...
        <div className="flex flex-col lg:flex-row gap-20 items-center">
          ...
          <div className="flex-1 w-full space-y-12">
            ...
            >
              ...
              >
                The AI Development <br />
                <span style={{ color: current.accentColor }} className="transition-colors duration-700">System.</span>
              </motion.h2>
            ...
            <div className="space-y-6">
              <p className="text-[10px] font-mono font-bold uppercase tracking-widest text-text-muted block border-b border-border-ghost pb-2 w-fit">Choose Your Approach</p>
